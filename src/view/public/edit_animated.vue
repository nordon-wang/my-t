  ...
  <div class="pub-animated">
    <div class="animated-img clearfix">
        ...
        :class="{ischeck:myAni.animatedType == item.animated,'img-items':index > 0}" @click="checkAni(item.animated,index)" >
          ...
          <dt v-if="index != 0 ? true : false" :style="{
             backgroundImage: `url(${picUrl})`
          }"></dt>
          <span class="ani-img-check" v-show="myAni.animatedType == item.animated" :style="{
             backgroundImage: `url(${picUrl})`
          }"></span>
          <span class="ani-img-check" v-show="myAni.animatedType != item.animated"></span>
          <dd>{{item.name}}</dd>
        </dl>
    ...
    <div class="animated-nums" style="text-align:center;">
      <div class="num-items">
        <strong>时间(s)</strong>
      ...
      <!-- <div class="num-items num-items-ul">
        <strong>触发</strong>
        <ul>
          <li  @click="countChange(false)" :class="{
            licheck:myAni.animatedIterationCount == false
          }" >单次触发</li>
          <li  @click="countChange(true)"  :class="{
            licheck:myAni.animatedIterationCount == true
          }" >多次触发</li>
        </ul>
      </div> -->

      <!-- <div class="num-items" v-show="showSecondAni">
        <strong>方向</strong>
        <el-radio-group v-model="myAni.animatedState" @change="animatedChange">
          <el-radio v-for="(item,index) in feiRuArr" @change="alert(111)" :key="index"  :label="item.animated">{{item.name}}</el-radio>
        </el-radio-group>
      </div> -->
      
      <div class="num-items num-items-ul" v-if="showSecondAni">
        <strong>方向</strong>
        <ul>
          <li v-for="(item,index) in feiRuArr" @click="animatedChange(item.animated)" :class="{
            licheck:item.animated == myAni.animatedState
...
export default {
    ...
    watch:{
      myAni:{
        handler: function (val, oldVal) {
          ...
          const moduleName = this.templet.p_nameToChinese(this.$store.state.module_Data.name)
          this.templet.module_add_historical(`编辑${moduleName}动画`);
        },
        deep: true
      },
    },
    ...
    mounted(){
      ...
      if(process.env.NODE_ENV === 'development' && my_path){
        // /index_pc.php/newapi/view/home/index/
