        ...
        <div  style="cursor: default;">
            <el-tabs v-model="activeName2" type="card" >
                <el-tab-pane label="配置管理" name="first">
                    ...
                    <el-row>
                    <el-col :span="12" >
                        <my-number class="msgedit" v-model="infor.placeholder_fs" :max='2000' :min='10' title="字体大小"></my-number>
                    </el-col>
                    <el-col :span="12" class="msgedit">
                        <my-color v-model="infor.placeholder_color" title="字体颜色"></my-color>
                    </el-col>
                    </el-row>
                </el-tab-pane>
                <el-tab-pane label="布局管理" name="formedits">
                <div id="msgLayout-items">
                    <ul>
                        <li>
                            <!-- <span >公共设置</span> -->
                            <el-button icon="el-icon-edit" size="mini" type="primary" @click="editPublic">公共设置</el-button>
                        </li>
                        <li v-for="(items,index) in infor.form_ziduan_arr" :key="index" v-on:click="editItem(index)">
                ...
                <!-- 拖拽容器 -->
                <div id="msgLayout-box" 
                :style="{width:infor.form_all_width+'px',height:infor.form_all_height+'px'}" 
                style=" border: 1px solid #333; position: relative;">
                    <div v-for="(items,index) in infor.form_ziduan_arr" v-on:dblclick="editItem(index)"  :key='index' :style="{
                    position: 'absolute',
                    width:items.styles.width + 'px',
                    height:items.styles.height+'px',
                    lineHeight:items.styles.height+'px',
                    left:items.styles.left+'px',
                    top:items.styles.top+'px',
                    backgroundColor:items.styles.backgroundColor,
                    cursor:'move',
                    color:items.styles.color,
                    textAlign:'center',
            ...
            <el-dialog title="表单编辑" 
                ...
                <component :is="getTypeComponent" :tem_datas="tem_datas" ></component>
...
export default {
  ...
  data() {
    ...
    methods:{
        // 添加按钮图片-- 存在infor中  悬浮
        // 添加按钮图片--对象中 默认
        // addDoubleBtnImg:function(item){
        //     var _this = this;
        //     templet.file_manage(function(d) {
        //         // EditVm.$set(item['btnImg'],d.path);
        //         _this.infor.form_select_datas[_this.edit_id].styles[item] = d.path;
        //     }, {
        //         app_id: 'site',
        //         radio: true
        //     });
        // },
        // removeDoubleBtnImg:function(item){
        //     this.infor.form_select_datas[this.edit_id].styles[item] = '';
        // },
        mystop(){
        ...
        modalOk:function(){
            this.infor.form_select_datas[this.edit_id] = this.infor.form_select_datas[this.edit_id];
        },
        ...
        randomColor:function(){
            ...
            num = '#'+num.toString(16);
            // console.log('颜色',num.length > 6 ? num : '#4c3085');
        ...
        createDatas:function(obj){
            ...
            $.each(obj,function(index,item){
                if (obj[index].styles) {
                    ...
                    obj[index].styles = {
                        width: ((obj[index].styles && obj[index].styles.width) || 130),
                        height:((obj[index].styles && obj[index].styles.height) || 30),
        ...
        init:function(){
            ...
            $.ajax({
              ...
              success(msg){
                    ...
                    // console.log('form_select_datas请求',_this.infor.form_select_datas);
              },
              error(err){
    ...
    mounted() {
        $("#edit_public").draggable({ handle: "div.edit_public_title" ,scroll:false});
        // 初始化表单
        this.init();
    },
    updated() {
...
//组件绑定拖拽
function componentDrag(obj) {
    $('#msgLayout-box>div').draggable({
        containment: "parent",
        zIndex: 100,
        snapTolerance:10,
        stop:function (event, ui) {
            ...
            console.log('组件绑定拖拽',num, obj.infor.form_select_datas[num].styles.left);
            
            obj.infor.form_select_datas[num].styles.left = ui.position.left;
            obj.infor.form_select_datas[num].styles.top = ui.position.top;
        }
    })
}

//组件初始化绑定缩放
function componentResizable(obj) {
    $('#msgLayout-box>div').resizable({
        containment: "parent",
        handles: "se",
        stop:function (event, ui) {
            // 操作的序列
            ...
            obj.infor.form_select_datas[num].styles.width = ui.size.width;
            obj.infor.form_select_datas[num].styles.height = ui.size.height;
        }
    })
}
</script>
...
#msgLayout-items ul li button {overflow: hidden;
white-space: nowrap;
text-overflow: ellipsis;
width: 92px;    padding: 7px 1px;
}
#msgLayout-items ul{
    white-space: normal;
}
