...
var RulersGuides = function(evt, dragdrop) {
    ...
    var doc = document.documentElement,
        ...
        guidesCnt = 0,
        gUid = '',
        rulerStatus = 0, //初始化标尺显示 0为隐藏
        guideStatus = 1, //初始化参考线显示 0 为隐藏
        hBound = 0,
        hBoundEnd = 0,
        vBound = 0,
        gridList = null,
            ...
            if (type === 'h') {
                var widthW =
                    ...
                    document.documentElement.clientWidth
                     // console.log($('.laybox').width())
                    //document.querySelector('.laybox')
               // var contentW = 898;
                var contentW = $('.laybox').width();
                var sideW = (widthW - contentW) / 4;
                i = -Math.floor(sideW);
                newsize -= sideW * 2;
                //debugger;
            }
        ...
        toggleRulers = function() {
            ...
            if (rulerStatus === 1) {
                ...
                wrapper.style.display = 'block';
                //标尺显示时颜色选择出现
                document.querySelector('.guideColor').style.display = 'block';
                removeInboundGuides();
            } else {
                vRuler.style.display = 'none';
                hRuler.style.display = 'none';
                document.querySelector('.guideColor').style.display = 'none';
            }
        },
        ...
        OpenGridDialog = function() {
            var dialog = null,
                ...
                renderSelect = function(insertOrUpdate) {
                    ...
                    //         if (e.target.className.indexOf("guide") > -1) {
        ...
        saveGrid = function() {
            ...
            while (gridName === '' && guidesCnt > 0) {
                gridName = window.prompt('填写参考线名称');
        ...
        showDetailedInfo = function() {
            ...
            for (i = 0; i < hGuides.length - 1; i = i + 1) {
                ...
                for (j; j < vGuides.length - 1; j = j + 1) {
                    ...
                    text = (vGuides[j + 1] - vGuides[j]) + ' x ' + (hGuides[i + 1] - hGuides[i]+2);

                    infoData1.nodeValue = text;

                    var widthW = (document.body.scrollWidth > document.documentElement.clientWidth) ?
                        document.body.scrollWidth :
                        document.documentElement.clientWidth
                    //var contentW = 898;
                    var contentW = $('.laybox').width();
                    var sideLW = Math.floor((widthW - contentW) / 2);

                    //text = (hGuides[i] + 2) + ' : ' + (vGuides[j] - sideLW + 3);
                    var startY = hGuides[i] + 2
                    if (startY==2) {
                        startY =0
                    }
                    text =  'X:'+(vGuides[j] - sideLW + 3)  + ' Y:' + startY ;
        ...
        Menu = function() {
            ...
            this.render = function() {
                ...
                let  menuBtnTip = document.createElement('p');
                menuBtnTip.className = 'menu-btn-tip';
                menuBtnTip.textContent = '标尺'
                //获取标尺icon的位置
                menuBtn.appendChild(menuBtnTip)
                let right = $("#top-save").width() + 10;
                $('.guideColor').css('right',right + 60)
                menuBtn.style.right = right + 'px';
                menuBtn.appendChild(document.createTextNode('\u250C'));

                menuList = document.createElement('ul');
                menuList.className = 'rg-menu';
                menuList.style.right = right + 'px';
                ...
                evt.attach('mousedown', menuBtn, function() {
                    ...
                    toggles.lock.txt.nodeValue = (locked === 0) ? '解锁标尺' : '锁定标尺';
                    //toggles.details.txt.nodeValue = (detailsStatus === 0) ? '显示详细信息' : '隐藏显示信息';
                    //toggles.snapdom.txt.nodeValue = (snapDom === 0) ? '开启对齐元素' : '关闭对齐元素';
        ...
        prepare = function() {
            ...
            setTimeout(function() {
                ...
                wrapper.style.width = (size[0] - 10) + 'px';
                wrapper.style.height = (size[1]) + 'px';
    ...
    evt.attach('mousedown', document, function(e, src) {
        ...
        if (e.target.className.indexOf('ruler') < 0) {
            return false;
        }
        ...
        if (
            (
                //判断画线开始的区域
                (x > vBound && y > hBound && y < hBoundEnd) ||
                (y > hBoundEnd && x < vBound)
            ) && rulerStatus === 1
        ) {
            guide = document.createElement('div');
            guideInfo = guide.cloneNode(false);
            guideInfoText = document.createTextNode('');
            //console.log(guideInfoText)
            //debugger;
            ...
            if (x > vBound && y < hBoundEnd && y > hBound) {
                guide.className = 'guide h draggable';
                guide.style.borderBottomColor = Store.state.guideColor;
                guide.style.top = (e.clientY - 60 + scrollPos[0]) + 'px';
                guideInfo.style.left = (x + scrollPos[1] + 10) + 'px';
                guide.type = 'h';
                snap = ySnap;
                mode = 2;
            } else if (y > hBoundEnd && x < vBound) {
                guide.className = 'guide v draggable';
                guide.style.borderRightColor = Store.state.guideColor;
            ...
            guide.addEventListener('dblclick', function(e) {
            ...
            //         console.log(e)
            //         if (
            //             (e.clientX > vBound && e.clientY > hBound && e.clientY < hBoundEnd) ||
            //             (e.clientY > hBoundEnd && e.clientX < vBound)
            //         ) {
            //             //console.log('不能画')
            //         } else {
            //             //console.log('能画')
